...
- 首页真机图：竖屏下班倒计时，不要灵动岛叠字，不要商店合成图。素材在 `assets/device/`：`en` / `zh` × `white` / `black` 各一张静帧，外加同场景短视频。语言跟内容语言走（中文门厅用 `zh`，其余用 `en`）；亮暗跟 `prefers-color-scheme`。第一版可以只放静帧。
- 第一版不放桌面主窗 / 迷你计时（窗口标题仍是旧名）。不上产品仓里那套旧名桌面 demo。iOS 计时短视频已经在本仓，要用再接。

### 下载页

与首页同一套三个入口，外加「为何要用原生」：通知、小组件/菜单栏、关了页面仍在走。第一版无录屏、无功能对比表。语气不贬网页版。不放 GitHub 直装。
...
- [x] 中英 × 浅色/深色真机静帧，以及同场景短视频（`assets/device/`）
